...
def dock_best_molecule(mol, receptor_path, receptor_center):
    ...
    try:
        ...
        # Create QVina command - ensure all arguments are strings
                ...
                for line in f:
                    if "REMARK VINA RESULT" in line:
                        try:
                            ...
                            # If SDF was created successfully, copy it to the same directory as the receptor
                            if os.path.exists(sdf_output):
                                receptor_dir = os.path.dirname(receptor_path)
                                final_sdf_path = os.path.join(receptor_dir, f"docked_{smiles.replace('/', '_')}_{score:.2f}.sdf")
...
class StatePool:
    ...
    def get_dataframe(self, num_calc_extra_metrics: int = 10) -> pd.DataFrame:
        ...
        # Generate CKA analysis plots if available
        if hasattr(self._model, 'cka_scores') and self._model.cka_scores:
            print("\n" + "🎨"*30)
            print("🎯 STARTING CKA PLOT GENERATION")
            print("🎨"*80)
            print("📊 Creating visual analysis of adapter ablation study results...")
            print("🎨"*80)
            
            try:
                # Create plots directory in the same location as output
                cka_dir = "./cka_analysis_plots"
                os.makedirs(cka_dir, exist_ok=True)
                
                # Generate plots
                self._model.analyze_cka_scores(save_plots=True, plot_dir=cka_dir)
                
                print("\n" + "✅"*30)
                print(f"🎉 CKA PLOTS SUCCESSFULLY GENERATED!")
                print(f"📁 Saved to: {cka_dir}")
                print("✅"*80)
            except Exception as e:
                print(f"\n❌ Error generating CKA plots: {str(e)}")
                print("❌"*40)
