...
class ClassifierHead(nn.Module):
    ...
    def predict(self, h: torch.Tensor) -> torch.Tensor:
        ...
        # Check if the logits have extreme values that could cause numerical issues
        logits_max = logits.max().item()
        logits_min = logits.min().item()
        logits_range = logits_max - logits_min
        
        # If the range is too large, normalize the logits to prevent numerical issues
        if logits_range > 1000:
            # Normalize by centering around the max value and scaling
            logits = logits - logits.max(dim=-1, keepdim=True)[0]
            # Limit the minimum value to prevent extreme negatives
