...
class Synformer(nn.Module):
    ...
    def encode(self, batch, mixture_weight: float | None = None):
        ...
        if hasattr(self, 'encoder_type') and self.encoder_type == "desert":
            ...
            if self.no_adapter:
                print("=== Bypassing Fragment Encoder (no_adapter) ===\n")
                fragment_ids = torch.tensor([item[0] for item in desert_sequence], dtype=torch.long, device=device).unsqueeze(0)
                code = self.desert_embedding.to(device)(fragment_ids)

                batch_size, seq_len = code.shape[0], code.shape[1]
