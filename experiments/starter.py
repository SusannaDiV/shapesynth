...
def visualize_all_pharmacophore_fingerprints(mol1, mol2, 
    ...
    two parent molecules, their blended child, and valid fragment combinations
    """
    try:
        import plotly.graph_objects as go
        from plotly.subplots import make_subplots
        
        # Generate fragment combinations and find ones with preserved features
        p1_fragments = fragment_molecule_with_dummy_atoms(mol1)
        p2_fragments = fragment_molecule_with_dummy_atoms(mol2)
        
        # Store valid combinations with their feature counts
        valid_combinations = []
        for frag1 in p1_fragments:
            for frag2 in p2_fragments:
                child_mol = connect_fragments_at_dummy_atoms(frag1, frag2)
                if child_mol:
                    # Process the child molecule
                    child_mol = process_mol(child_mol, Chem.MolToSmiles(child_mol))
                    if child_mol:
                        # Check pharmacophore features
                        features = identify_pharmacophore_features(child_mol)
                        if features:
                            # Count total features
                            feature_count = sum(len(feat_list) for feat_list in features.values())
                            valid_combinations.append((child_mol, feature_count))
        
        # Sort combinations by feature count and get top 2
        valid_combinations.sort(key=lambda x: x[1], reverse=True)
        top_combinations = valid_combinations[:2] if len(valid_combinations) >= 2 else valid_combinations
        ...
        # Add molecule structures and pharmacophore features for parents and blended
        ...
        # Add structures and features for top combinations
        for i, (combo_mol, _) in enumerate(top_combinations):
            # Generate pharmacophore fingerprints for the combination
            combo_grid, combo_features, _, combo_info = generate_pharmacophore_fingerprints(combo_mol)
            if combo_grid is not None:
                add_molecule_structure(combo_mol, 1, i + 4)
                add_pharmacophore_features(combo_features, combo_info, 1, i + 4)
        
        # Update layout
        fig.update_layout(
            title="Pharmacophore Fingerprints and Valid Combinations",
            width=2400,  # Increased width to accommodate additional subplots
            height=700,
            legend=dict(
                x=0,
                y=1,
...
def create_simplified_visualization(mol1, mol2, 
    ...
    try:
        ...
        for i, (combo_mol, _) in enumerate(top_combinations):
            ...
            if combo_grid is not None:
                add_molecule_structure(combo_mol, 1, i + 4)
                add_pharmacophore_scatter(combo_features, combo_info, 1, i + 4)
...
def main():
    ...
    # Visualize fragment combinations
    print("\nVisualizing fragment combinations...")
    visualize_fragment_combinations(mol1, mol2)
    
    # Score fragment combinations and print ranking
    #print("\nScoring fragment combinations...")
    #score_fragment_combinations(mol1, mol2, blended_overall, aligned_grid_info)
...
def visualize_fragment_combinations(parent1, parent2, filename="fragment_combinations.html"):
    """
    Visualize combinations of fragments containing pharmacophore features from both parents.
    """
    p1_fragments = fragment_molecule_with_dummy_atoms(parent1)
    p2_fragments = fragment_molecule_with_dummy_atoms(parent2)
    ...
    for frag1 in p1_fragments:
        for frag2 in p2_fragments:
            child_mol = connect_fragments_at_dummy_atoms(frag1, frag2)
            if child_mol:
                valid_smiles = Chem.MolToSmiles(child_mol)
                print(f"✅ Valid combination: {valid_smiles}")
                valid_combinations.append(valid_smiles)

    with open(filename, "w") as f:
        for smiles in valid_combinations:
            f.write(smiles + "\n")

    print(f"Fragment combinations visualization saved to {filename}")

def calculate_gaussian_overlap_from_grids(grid1, grid2):
    """
    Calculate Gaussian shape overlap (Tanimoto similarity) directly from two precomputed grids.
    Handles zero values properly to avoid division by zero.
    """
    grid1_flat = grid1.flatten()
    grid2_flat = grid2.flatten()

    # Check if either grid is all zeros
    if np.all(grid1_flat == 0) or np.all(grid2_flat == 0):
        print("Warning: One or both grids contain all zeros")
        return 0.0

    # Normalize grids
    sum1 = np.sum(grid1_flat)
    sum2 = np.sum(grid2_flat)
    
    if sum1 > 0:
        grid1_flat = grid1_flat / sum1
    if sum2 > 0:
        grid2_flat = grid2_flat / sum2

    intersection = np.sum(np.minimum(grid1_flat, grid2_flat))
    union = np.sum(np.maximum(grid1_flat, grid2_flat))

    if union <= 0:
        print("Warning: Union of grids is zero")
        return 0.0

    return intersection / union

def score_fragment_combinations(parent1, parent2, blended_overall, blended_grid_info, filename="scored_fragment_combinations.txt"):
    """
    Score fragment combinations based on shape similarity with the blended parent shape.
    """
    p1_fragments = fragment_molecule_with_dummy_atoms(parent1)
    p2_fragments = fragment_molecule_with_dummy_atoms(parent2)

    print(f"\nScoring {len(p1_fragments)} x {len(p2_fragments)} = {len(p1_fragments) * len(p2_fragments)} possible combinations...")
    scored_combinations = []

    for i, frag1 in enumerate(p1_fragments):
        for j, frag2 in enumerate(p2_fragments):
            print(f"\nEvaluating combination {i+1}-{j+1}:")
            print(f"Fragment 1 SMILES: {Chem.MolToSmiles(frag1)}")
            print(f"Fragment 2 SMILES: {Chem.MolToSmiles(frag2)}")
            ...
            if child_mol:
                try:
                    ...
                    child_mol = process_mol(child_mol, Chem.MolToSmiles(child_mol))
                    if child_mol:
                        print(f"Valid child SMILES: {Chem.MolToSmiles(child_mol)}")
                        
                        # Generate child grid
                        print("Generating child grid...")
                        child_grid, _, _, child_grid_info = generate_pharmacophore_fingerprints(child_mol)
                        
                        if child_grid is not None:
                            print("Child grid shape:", child_grid.shape)
                            print("Child grid sum:", np.sum(child_grid))
                            print("Child grid max:", np.max(child_grid))
                            
                            print("Aligning child grid with blended grid...")
                            aligned_child_grid, _, _, _, _ = align_pharmacophore_grids(
                                child_grid, {}, child_grid_info,
                                blended_overall, {}, blended_grid_info
                            )
                            
                            print("Aligned child grid shape:", aligned_child_grid.shape)
                            print("Aligned child grid sum:", np.sum(aligned_child_grid))
                            print("Aligned child grid max:", np.max(aligned_child_grid))
                            print("Blended overall grid shape:", blended_overall.shape)
                            print("Blended overall grid sum:", np.sum(blended_overall))
                            print("Blended overall grid max:", np.max(blended_overall))

                            # Calculate shape similarity
                            shape_similarity = calculate_gaussian_overlap_from_grids(aligned_child_grid, blended_overall)
                            print(f"Shape similarity score: {shape_similarity:.3f}")

                            smiles = Chem.MolToSmiles(child_mol)
                            scored_combinations.append((smiles, shape_similarity))
                            
                except Exception as e:
                    print(f"Error processing combination: {str(e)}")
                    continue

    # Sort combinations by similarity score
    scored_combinations.sort(key=lambda x: x[1], reverse=True)

    # Save all results with detailed information
    with open(filename, "w") as f:
        f.write("SMILES\tShape Similarity\n")
        for smiles, similarity in scored_combinations:
            f.write(f"{smiles}\t{similarity:.6f}\n")

    print(f"\nScored {len(scored_combinations)} valid combinations")
    print(f"Results saved to {filename}")

    print(f"Scored fragment combinations saved to {filename}")
