...
def visualize_all_pharmacophore_fingerprints(mol1, mol2, 
    ...
    try:
        ...
        # Create a figure with four subplots (three for molecules, one for combinations)
        fig = make_subplots(
            rows=1, cols=4,
        ...
        # Generate and add valid fragment combinations
        ...
        for frag1 in p1_fragments:
            for frag2 in p2_fragments:
                ...
                if child_mol:
                    ...
                    if child_mol:
                        # Check if it contains pharmacophore features
                        features = identify_pharmacophore_features(child_mol)
                        if features:
                            valid_combinations.append(child_mol)
        
        # Add valid combinations to the fourth subplot
        if valid_combinations:
            # Display up to 3 valid combinations
            for i, child_mol in enumerate(valid_combinations[:3]):
                # Add the molecule structure with a vertical offset
                conf = child_mol.GetConformer()
                positions = []
                elements = []
                
                # Calculate vertical offset
                offset = i * 5.0  # Offset each molecule by 5 units
                
                for atom in child_mol.GetAtoms():
                    idx = atom.GetIdx()
                    pos = conf.GetAtomPosition(idx)
                    positions.append([pos.x, pos.y, pos.z + offset])
                    elements.append(atom.GetSymbol())
        ...
        fig.update_layout(
            ...
            width=2400,  # Increased width to accommodate fourth subplot
            height=700,
            legend=dict(
                x=0,
                y=1,
