...
def generate_valid_child_molecule(parent1, parent2):
    ...
    '''
    if not best_child:
        print("No valid child molecules generated")
        return None
    return best_child
    ...
    try:
        ...
        try:
            ...
            if clean_mol1 and clean_mol2:
                ...
                if brics_bonds:
                    # Fragment the molecules using BRICS
                    frags1 = list(BRICS.BRICSDecompose(clean_mol1))
                    frags2 = list(BRICS.BRICSDecompose(clean_mol2))
                    
                    # Combine fragments
                    all_frags = frags1 + frags2
                    
                    # Try to reconnect using BRICS
                    try:
                        # Use CloseMolRings to reconnect fragments
                        brics_mol = BRICS.CloseMolRings(all_frags)
...
def dock_molecule(mol, receptor_path, center, box_size, qvina_path="bin/qvina2.1", obabel_path="obabel"):
    """
...
def select_best_conformer(mol, parent1, parent2, receptor_path, center, box_size, qvina_path="bin/qvina2.1", obabel_path="obabel"):
    """
    Select the best conformer based on docking score
    
    Args:
        mol: Child molecule to generate conformers for
        parent1, parent2: Parent molecules (kept for function signature compatibility)
    ...
    Returns:
        best_mol: RDKit molecule with best docking conformer
        best_score: Best docking score
    """
    ...
    # For each conformer, calculate docking score
    for conf_id, energy in conformer_results:
        try:
            ...
            # Dock this conformer
            print(f"\nDocking conformer {conf_id}...")
            score = dock_molecule(conf_mol, receptor_path, center, box_size, qvina_path, obabel_path)
            
            if score is not None:
                print(f"Conformer {conf_id}: Docking score = {score:.2f} kcal/mol, Energy = {energy:.2f}")
                
                if score < best_score:
                    best_score = score
                    best_conformer_id = conf_id
                    print(f"New best conformer found: {conf_id} (score: {score:.2f} kcal/mol)")
                
        except Exception as e:
            print(f"Error processing conformer {conf_id}: {str(e)}")
            continue
    
    if best_conformer_id is not None:
        # Create a molecule with only the best conformer
        best_mol = Chem.Mol(mol)
        best_mol.RemoveAllConformers()
        best_mol.AddConformer(mol.GetConformer(best_conformer_id))
        print(f"\nSelected best conformer {best_conformer_id} with docking score {best_score:.2f} kcal/mol")
...
def main():
    ...
    if child_mol:
        # Select best conformer based on docking score
        print("\nSelecting best conformer...")
        best_mol, child_score = select_best_conformer(
            child_mol, mol1, mol2,
        ...
        if best_mol is not None:
            print(f"\nBest Child Conformer ({Chem.MolToSmiles(best_mol)})")
    ...
    if child_score is not None:  # Only include child if valid
        synformer_smiles.append(Chem.MolToSmiles(best_mol))
