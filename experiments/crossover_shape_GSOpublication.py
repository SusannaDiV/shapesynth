...
def is_fragment_bulky(fragment, max_atoms=20, max_rings=2):
    """Check if a fragment is too bulky."""
    num_atoms = fragment.GetNumAtoms()
    num_rings = len(Chem.GetSymmSSSR(fragment))
    return num_atoms > max_atoms or num_rings > max_rings

def validate_molecule(mol, parent1, parent2):
    """Validate molecule against multiple criteria."""
    if mol is None:
    ...
    if tpsa > 140:
        return False, "TPSA too high"
    
    # Check LogP
    logp = Chem.Crippen.MolLogP(mol)
    if logp > 6:
        return False, "LogP too high"
    
    # Count pharmacophore matches from parents
    parent_features1 = identify_pharmacophore_features(parent1)
    parent_features2 = identify_pharmacophore_features(parent2)
    child_features = identify_pharmacophore_features(mol)
    
    # Count matches between child and parent features
    matches = 0
    for feat in child_features:
        if feat in parent_features1 or feat in parent_features2:
            matches += 1
    
    if matches <= 3:
        return False, "Insufficient pharmacophore matches"
    
    return True, "Valid molecule"

def get_fragment_pharmacophore_count(fragment):
    """Count pharmacophore features in a fragment."""
    features = identify_pharmacophore_features(fragment)
    return len(features)

def generate_valid_child_molecule(parent1, parent2):
    """Generate valid child molecules considering pharmacophore features and constraints."""
    for attempt in range(10):  # Try up to 10 times
        # Fragment both parents
        frags1 = fragment_molecule_with_dummy_atoms(parent1)
        frags2 = fragment_molecule_with_dummy_atoms(parent2)
        
        if frags1 is None or frags2 is None:
            continue
        
        # Score fragments by pharmacophore features
        frag_scores1 = [(f, get_fragment_pharmacophore_count(f)) for f in frags1]
        frag_scores2 = [(f, get_fragment_pharmacophore_count(f)) for f in frags2]
        
        # Sort by pharmacophore count
        frag_scores1.sort(key=lambda x: x[1], reverse=True)
        frag_scores2.sort(key=lambda x: x[1], reverse=True)
        
        # Check for bulky fragments
        for frag1, score1 in frag_scores1:
            for frag2, score2 in frag_scores2:
                if is_fragment_bulky(frag1) and is_fragment_bulky(frag2):
                    continue
                
                # Try to connect fragments
                child = connect_fragments_at_dummy_atoms(frag1, frag2)
                if child is None:
                    continue
                
                # Validate the child molecule
                is_valid, reason = validate_molecule(child, parent1, parent2)
                if not is_valid:
                    continue
                
                # Generate conformers
                child = Chem.AddHs(child)
                conf_ids = AllChem.EmbedMultipleConfs(child, numConfs=10, 
                                                    params=AllChem.ETKDGv3())
                
                if len(conf_ids) == 0:
                    continue
                
                # Minimize each conformer using MMFF94
                for conf_id in conf_ids:
                    try:
                        # Try MMFF first
                        if AllChem.MMFFOptimizeMolecule(child, confId=conf_id) == 0:
                            continue
                        # Fall back to UFF if MMFF fails
                        if AllChem.UFFOptimizeMolecule(child, confId=conf_id) != 0:
                            continue
                    except:
                        continue
                ...
                for conf_id in conf_ids:
                    try:
                        # Try MMFF94 first
                        mp = AllChem.MMFFGetMoleculeProperties(child)
                        ff = AllChem.MMFFGetMoleculeForceField(child, mp, confId=conf_id)
                        if ff is None:
                            # Fall back to UFF
                            ff = AllChem.UFFGetMoleculeForceField(child, confId=conf_id)
...
def generate_and_optimize_conformers(mol, num_conformers=3):  # Generate 3 conformers as requested
    """
...
def main():
    # Example usage
    smiles1 = "CN(CCCN1CCN(CC1)CCc2ccc(cc2)Cl)CC3=CC=CC=C3"  # Acetaminophen
    smiles2 = "CC(C)NC[C@H](O)COc1cccc(c1)Cc2ccc(cc2)COCCCN"  # Ibuprofen
