...
def reproduce(mating_pool, mutation_rate):
    ...
    try:
        ...
        # If crossover fails, return one of the parents
        print(f"Crossover failed: {str(e)}. Returning parent.")
        return parent_a
...
if __name__ == "__main__":
    ...
    # Use ADRB2 molecules as starting population
    starting_population = adrb2_smiles
    population_smiles = starting_population
    population_smiles = projection(population_smiles)
    ...
    while True:
        ...
        population_mol = [
            Chem.MolFromSmiles(smi) for smi in projection([Chem.MolToSmiles(mol) for mol in population_mol])
        ]
