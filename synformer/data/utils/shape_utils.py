...
from math import ceil, pi
from synformer.data.utils.tfbio_data import make_grid, ROTATIONS
import random
import copy
from skimage.util import view_as_blocks
from  synformer.data.utils.tfbio_data  import rotation_matrix
import pickle as pkl
from concurrent.futures import TimeoutError
from pebble import ProcessPool, ProcessExpired
import math
from rdkit.Chem.AllChem import AlignMol
from synformer.data.utils.molecule_preparation import MoleculePreparation,  get_rings, get_other_atom_idx, find_parts_bonds
...
vocab_path = '/home/luost_local/sdivita/synformer/data/desert/vocab.pkl'
...
def hypos_to_mols(token_hypos, trans_hypos, rotat_hypos):
    mols = []
    for token_hypo, trans_hypo, rotat_hypo in zip(token_hypos, trans_hypo, rotat_hypo):
        m = []
...
def dock(op_results, receptor, ref):
    ...
    for mol, idx in op_results:
        try:
            ...
            # Convert RDKit mol to PDBQT using Meeko's preparation pipeline
            mol = Chem.AddHs(mol, addCoords=True)
            prep = MoleculePreparation(
                charges_from_mol=True,  # Use charges from input mol
                rigid_macrocycles=True,  # Keep macrocycles rigid
                min_ring_size=3  # Minimum ring size to be considered
            )
            
            # Prepare the molecule directly from RDKit mol
            prep.prepare(mol)
