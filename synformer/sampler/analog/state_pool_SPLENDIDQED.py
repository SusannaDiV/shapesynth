        ...
        for i, j in nm_iter:
            ...
            if tok_next == TokenType.END:
                ...
                if success:
                    ...
                    # Calculate combined score: 70% docking + 30% QED
                    if docking_scores:
                        ...
                        # Use QED in combined score
                        combined_score = 0.8 * max_docking + 0.2 * (1 if max_qed > 0.5 else 0)
                        print(f"Combined score: {combined_score:.4f} (Docking: {max_docking:.4f} [raw: {raw_docking:.2f} for {best_smiles}], QED > 0.5: {max_qed > 0.5})")
                        ...
                        # If docking failed, use only QED
                        combined_score = (1 if max_qed > 0.5 else 0) # Use QED condition
                        print(f"Using only QED score: {combined_score} (QED > 0.5: {max_qed > 0.5}, docking failed)")
                        
                        # Save good molecules based on QED alone
                        if combined_score >= 0.5:
                            self._good_molecules.append({
                                "target": self._mol.smiles,
                                "smiles": best_smiles if 'best_smiles' in locals() else top_mols[0].smiles if top_mols else "N/A", # Handle if best_smiles not defined
                                "qed_score": max_qed, # Store QED
                                "docking_score": None,
                                "score": combined_score,
                                "synthesis": new_state.stack.get_action_string(),
                            ...
                            print(f"Saved molecule with QED score: {combined_score}")
                        
                        # Continue only if QED score is good enough
                        if combined_score >= 0.6:
                            new_state.scores.append(combined_score)
                            next.append(new_state)
                            print(f"Continuing reaction pathway with QED score: {combined_score}")
                        else:
                            self._aborted.append(new_state)
                            print(f"Aborting reaction pathway with low QED score: {combined_score}")
                else:
                    self._aborted.append(new_state)
    ...
    def get_dataframe(self, num_calc_extra_metrics: int = 10) -> pd.DataFrame:
        ...
        for product in self.get_products():
            ...
            if docking_score is not None:
                ...
                normalized_docking_score = max(0, min(1, (-docking_score) / 12.0))
                combined_score = 0.8 * normalized_docking_score + 0.2 * (1 if qed_score > 0.5 else 0)
                print(f"Combined score: {combined_score:.4f} (Normalized docking: {normalized_docking_score:.4f}, QED > 0.5: {qed_score > 0.5})")
            else:
                combined_score = (1 if qed_score > 0.5 else 0)
                print(f"Using only QED score: {combined_score} (QED > 0.5: {qed_score > 0.5}, docking failed)")
