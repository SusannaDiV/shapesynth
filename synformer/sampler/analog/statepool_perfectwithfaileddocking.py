            ...
            if tok_next == TokenType.END:
                ...
                if success:
                    ...
                    if docking_scores:
                        ...
                        # Save molecules that failed docking
                        if not docking_scores: # This means docking failed for all molecules in this branch
                            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                            filename = f"worsedocking_{timestamp}.csv"
                            # Create a list of dicts for the molecules that failed docking
                            failed_docking_mols_data = []
                            for idx, rdkit_mol_obj in enumerate(rdkit_mols): # Iterate through original rdkit_mols
                                smiles_str = mol_smiles_list[idx]
                                current_qed = 0.0
                                try:
                                    # Recalculate QED for this specific molecule if needed, or retrieve if stored
                                    # For simplicity, let's assume we can get it from top_mols based on smiles
                                    mol_obj = next((m for m in top_mols if m.smiles == smiles_str), None)
                                    if mol_obj:
                                        temp_rdkit_mol = Chem.MolFromSmiles(mol_obj.smiles)
                                        if temp_rdkit_mol:
                                            current_qed = QED.qed(temp_rdkit_mol)
                                except Exception as e:
                                    print(f"Error calculating QED for {smiles_str} during failed docking save: {str(e)}")

                                failed_docking_mols_data.append({
                                    "target": self._mol.smiles,
                                    "smiles": smiles_str,
                                    "qed_score": current_qed,
                                    "docking_score": "N/A (Failed)",
                                    "score": 0.0,
                                    "synthesis": new_state.stack.get_action_string(),
                                    "num_steps": new_state.stack.count_reactions(),
                                })
                            
                            if failed_docking_mols_data:
                                df_failed = pd.DataFrame(failed_docking_mols_data)
                                if not os.path.exists(filename):
                                    df_failed.to_csv(filename, index=False)
                                else:
                                    df_failed.to_csv(filename, mode='a', header=False, index=False)
                                print(f"Saved {len(failed_docking_mols_data)} molecules from failed docking attempts to {filename}")
