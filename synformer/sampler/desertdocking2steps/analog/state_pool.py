    ...
    def evolve(
        ...
        for i, j in nm_iter:
            ...
            if tok_next == TokenType.END:
                ...
                # Dock the reactant before adding it
                try:
                    rdkit_mol = Chem.MolFromSmiles(reactant.smiles)
                    if rdkit_mol is not None:
                        ...
                        if docking_score is not None:
                            ...
                            # Combine with model's score (50% docking, 50% model score)
                            combined_score = 0.5 * normalized_docking_score + 0.5 * score
                            print(f"Reactant docking score: {docking_score:.4f}, Combined: {combined_score:.4f}")
                        else:
                            combined_score = score
                            print(f"Reactant docking failed, using model score: {score:.4f}")
                    else:
                        combined_score = score
                        print(f"Failed to convert reactant to RDKit, using model score: {score:.4f}")
                except Exception as e:
                    print(f"Error docking reactant: {str(e)}")
                    combined_score = score
                ...
                # Get potential reactants for this reaction
                potential_reactants = self._rxn_matrix.get_reactants(reaction)
                
                # Dock each potential reactant
                best_reactant_score = 0
                best_reactant = None
                for reactant in potential_reactants:
                    try:
                        rdkit_mol = Chem.MolFromSmiles(reactant.smiles)
                        if rdkit_mol is not None:
                            docking_score = dock_best_molecule(rdkit_mol)
                            if docking_score is not None:
                                normalized_docking_score = max(0, min(1, (-docking_score) / 12.0))
                                # Combine with model's score
                                combined_score = 0.5 * normalized_docking_score + 0.5 * score
                                if combined_score > best_reactant_score:
                                    best_reactant_score = combined_score
                                    best_reactant = reactant
                                    print(f"Found better reactant with docking score: {docking_score:.4f}, Combined: {combined_score:.4f}")
                            else:
                                if score > best_reactant_score:
                                    best_reactant_score = score
                                    best_reactant = reactant
                                    print(f"Using model score for reactant: {score:.4f}")
                    except Exception as e:
                        print(f"Error docking reactant: {str(e)}")
                        if score > best_reactant_score:
                            best_reactant_score = score
                ...
                if best_reactant is not None:
                    ...
                    if success:
                        ...
                        if docking_scores:
                            ...
                            # Only continue reaction pathways that have good scores
                            if combined_score >= 0.5:
                                new_state.scores.append(combined_score)
                                next.append(new_state)
                                print(f"Continuing reaction pathway with score: {combined_score:.4f}")
                            else:
                                self._aborted.append(new_state)
                                print(f"Aborting reaction pathway with low score: {combined_score:.4f}")
                        else:
                            ...
                            # Save good molecules based on Tanimoto alone
                            if combined_score >= 0.5:
                                self._good_molecules.append({
                                    "target": self._mol.smiles,
                                    "smiles": best_smiles,
                                ...
                                print(f"Saved molecule with Tanimoto score: {combined_score:.4f}")
                            
                            # Continue only if Tanimoto score is good enough
                            if combined_score >= 0.5:
                                new_state.scores.append(combined_score)
                                next.append(new_state)
                                print(f"Continuing reaction pathway with Tanimoto score: {combined_score:.4f}")
                            else:
                                self._aborted.append(new_state)
                                print(f"Aborting reaction pathway with low Tanimoto score: {combined_score:.4f}")
                    else:
                        self._aborted.append(new_state)
                else:
