...
CAVITY_DIR = "/home/luost_local/sdivita/correct_rxnflow/RxnFlow/experiments/data/experiments/CrossDocked2020/protein/test/cavity"
RECEPTOR_DIR = "/home/luost_local/sdivita/correct_rxnflow/RxnFlow/experiments/data/experiments/CrossDocked2020/protein/test"
OUTPUT_DIR = os.path.join(BASE_DIR, "data/desert/sample_shapes_ablation_study")
print("ok")
...
with open(data_path, 'rb') as fr:
    data = pkl.load(fr)
# print(f"Loaded {len(data)} molecules from data file")

# print("Creating atom stamp...")
atom_stamp = get_atom_stamp(grid_resolution=0.5, max_dist=4.0)
# print("Atom stamp created")
...
def process_batch(batch_files):
    ...
    for cavity_file in batch_files:
        ...
        output_basename = cavity_basename.replace("_cavity.pdb", "")
        
        print(f"\nProcessing: {cavity_basename}")
        
        # Get corresponding receptor file
        pdb_id = cavity_basename.replace("_cavity.pdb", "")
        protein_file = os.path.join(RECEPTOR_DIR, f"{pdb_id}.pdb")
        
        if not os.path.exists(protein_file):
            print(f"Warning: No matching receptor file found for {cavity_file}")
            continue
            
        # Process the molecule pair
...
def main():
    # Get all cavity files
    cavity_files = glob.glob(os.path.join(CAVITY_DIR, "*_cavity.pdb"))
    ...
    unprocessed_files = [f for f in cavity_files 
                        if os.path.basename(f).replace("_cavity.pdb", "") not in processed_files]
