...
CSV_PATH = "fragments_qed_docking_with_latents.csv" # Make sure this is in /workspace/synformer/synformer/ or adjust path
...
class OracleShiftDataset(Dataset):
    def __init__(self, csv_path, qed_improvement_threshold=0.1, min_qed_for_target_search=0.0, max_qed_considered=0.95):
        print(f"Loading dataset from {csv_path} for oracle shift calculation...")
        ...
        for i in tqdm(range(len(raw_latents))):
    ...
    def __getitem__(self, idx):
        return {"latent_input": self.input_latents[idx], "target_delta_z": self.target_delta_zs[idx]}


# --- Decoder and Generation Functions (NOT USED IN THIS TRAINING LOOP) ---
...
def main():
    ...
    print(f"Attempting to load dataset from: {csv_full_path}")
    try:
        dataset = OracleShiftDataset(
            csv_full_path, 
    ...
    if len(dataset) == 0:
        print("Dataset is empty after processing. Exiting.")
        return
        
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=0)
    
    # Infer latent_dim from the first item
    first_item = dataset[0]["latent_input"]
    if first_item is not None:
        latent_dim = first_item.shape[0]
        print(f"Inferred latent dimension: {latent_dim}")
    else: # Should not happen if dataset loading was successful
        print(f"Using default latent dimension: {latent_dim}. Could not infer from dataset.")
        return
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
            ...
            if batch_idx % (len(dataloader) // 4 + 1) == 0 and batch_idx > 0 : # Log ~4 times per epoch
                 tqdm.write(f"Epoch {epoch+1}/{num_epochs} - Batch {batch_idx}/{len(dataloader)-1} - Current Batch Loss: {loss.item():.6f}")
